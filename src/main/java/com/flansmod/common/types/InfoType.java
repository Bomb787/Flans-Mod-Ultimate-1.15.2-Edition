...
public abstract class InfoType {
	...
	protected void read(String[] split, TypeFile file) {
		...
		try {
			...
			FlansMod.LOGGER.info("Reading file failed : " + shortName);
			e.printStackTrace();
	...
	public void addRecipe(Item par1Item) {
		...
		try {
			...
			FlansMod.LOGGER.info("Failed to add recipe for : " + shortName);
	...
	public static ItemStack getRecipeElement(String s, int amount, int damage, String requester) {
		...
		if (s.equals("doorWood")) {
			...
			return new ItemStack(Items.OAK_DOOR, amount);
		...
		for(Object object : ForgeRegistries.ITEMS) {
			
			Item item = (Item)object;
			if (item != null && item.getTranslationKey() != null && (item.getTranslationKey().equals("item." + s) || item.getTranslationKey().equals("tile." + s))) {
		...
		for(InfoType type : infoTypes) {
			...
			if(type.shortName.equals(s))
				return new ItemStack(type.Items, amount, damage);
		...
		FlansMod.LOGGER.info("Could not find " + s + " when adding recipe for " + requester);
		return null;
	...
	public static Effect getPotionEffect(String[] split) {
		
		int potionID = Integer.parseInt(split[1]);
		int duration = Integer.parseInt(split[2]);
		int amplifier = Integer.parseInt(split[3]);
		return new Effect([potionID, duration, amplifier], false);
